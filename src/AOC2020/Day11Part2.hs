module AOC2020.Day11Part2 where
...
stepSM sm = imap f sm
  where
    f k = stepSpot (visibleSpots sm k)

visibleSpots :: SM -> (Int, Int) -> [Spot]
visibleSpots sm c = mapMaybe (firstSpot sm c) directions
...
firstSpot :: SM -> (Int, Int) -> (Int, Int) -> Maybe Spot
firstSpot sm c dir =
  let c' = addC c dir
  in  case sm CV.!? c' of
    Just Floor -> firstSpot sm c' dir
    s -> s
...
stepSpot adj Full  | countFull adj >= 5 = Empty
stepSpot adj s = s
